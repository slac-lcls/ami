...
class NodeSourceParser:
    ...
    def extract_node_info(self, class_node, module_name):
        ...
        for item in class_node.body:
            ...
            # Look for __init__ to extract terminals
            if isinstance(item, ast.FunctionDef) and item.name == "__init__":
                terminals_info = self.extract_terminals_from_init(item)
    ...
    def extract_terminals_from_init(self, init_node):
        """Extract terminal names from __init__ method."""
        terminals = {}

        # Look for super().__init__(..., terminals={...}) calls
    ...
    def parse_terminals_dict(self, dict_node):
        ...
        for key, value in zip(dict_node.keys, dict_node.values):
            ...
            # Extract io type from value dict
            term_io = "unknown"
            ...
            terminals[term_name] = {"name": term_name, "io": term_io}
    ...
    def parse_add_terminal_call(self, call_node):
        ...
        # Look for io='in' or io='out' keyword
    ...
    def generate_terminals_quick_ref_md(self, output_path):
        ...
        for node_info in all_nodes:
            ...
            if not terminals:
                ...
                inputs = [k for k, v in terminals.items() if v.get("io") == "in"]
                outputs = [k for k, v in terminals.items() if v.get("io") == "out"]

                inputs_str = (
                    ", ".join(f"`{t}`" for t in sorted(inputs)) if inputs else "-"
                )
                outputs_str = (
                    ", ".join(f"`{t}`" for t in sorted(outputs)) if outputs else "-"
                )

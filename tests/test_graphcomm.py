...
@pytest.mark.parametrize('inputs, outputs, expected, graph_comm_simple',
                         [
                            (['test_in'], ['test_out'], False, True),
                            (['test_in'], ['test_out'], True, False),
                            ([gn.Var('test_in')], [gn.Var('test_out')], True, True),
                            ([gn.Var('test_in')], [gn.Var('test_out')], True, False),
                            ([gn.Var('test_in')], ['test_out'], False, True),
                            ([gn.Var('test_in')], ['test_out'], True, False),
                            (['test_in'], [gn.Var('test_out')], False, True),
                            (['test_in'], [gn.Var('test_out')], True, False),
                         ],
                         indirect=['graph_comm_simple'])
def test_enforce_types(inputs, outputs, expected, graph_comm_simple):
    comm, addr = graph_comm_simple
    try:
        comm._make_node(gn.Map, name="test", inputs=inputs, outputs=outputs, func=lambda x: x)
        # if a TypeError is expected we shouldn't get here
        assert expected
    except TypeError:
        # if a TypeError is not expected we shouldn't get here
        assert not expected

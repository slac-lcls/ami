...
if hasattr(psana, "_psana"):
    ...
    def mask_halfplane(shape, r1, c1, r2, c2, rm, cm, dtype=DTYPE_MASK):
        ...
        cond = (
            (r > r1 if rm < r1 else r < r1)
            if r1 == r2
            else (
                (c > c1 if cm < c1 else c < c1)
                if c1 == c2
                else ((r > r1 + f * (c - c1)) if signgt else (r < r1 + f * (c - c1)))
            )
        )
        # rm, cm = int(rm), int(cm)
    ...
    def mask_arc(shape, cx, cy, ro, ri, ao, ai, dtype=DTYPE_MASK):
        ...
        """
        logger.debug(
            "shape:%s  cx:%.2f  cy:%.2f  ro:%.2f  ri:%.2f  ao:%.2f  ai:%.2f" % (str(shape), cx, cy, ro, ri, ao, ai)
        )
        from math import cos, radians, sin  # floor, ceil

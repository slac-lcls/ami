...
def test_roi(TNAME):
    ...
    roi = (
        ur.ArchROI((50, 50), 100)
        if TNAME == "1"
        else (
            pg.TriangleROI((50, 50), 100)
            if TNAME == "2"
            else (
                pg.CrosshairROI((50, 50), 20)
                if TNAME == "3"
                else (
                    pg.EllipseROI((100, 100), 50)
                    if TNAME == "4"
                    else (
                        pg.RectROI((50, 50), (50, 70))
                        if TNAME == "5"
                        else (
                            pg.CircleROI((90, 90), 50)
                            if TNAME == "6"
                            else (
                                pg.LineROI((20, 10), (100, 90), 3)
                                if TNAME == "7"
                                else pg.PolyLineROI([(10, 20), (30, 10), (10, 50), (80, 90)], closed=True)
                            )
                        )
                    )
                )

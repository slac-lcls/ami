...
class PvExport(CtrlNode):
    ...
    nodeName = "PvExport"
    uiTemplate = [
        ("alias", "text", {"tip": "PV name to export variable under."}),
        ("events", "intSpin", {"value": 2, "min": 2, "tip": "Number of events/heartbeat to export"}),
    ]

    def __init__(self, name):
        super().__init__(
            name, terminals={"In": {"io": "in", "ttype": Any}, "eventid": {"io": "in", "ttype": int}}, exportable=True
        )
...
class ZMQ(CtrlNode):
    ...
    def __init__(self, name):
        super().__init__(
            name,
            terminals={"Timestamp": {"io": "in", "ttype": float}, "In": {"io": "in", "ttype": Any}},
            allowAddInput=True,
            viewable=True,
        )
...
class UDPMcast(CtrlNode):
    ...
    def __init__(self, name):
        super().__init__(
            name,
            terminals={"In": {"io": "in", "removable": False, "ttype": Any}, "eventid": {"io": "in", "ttype": int}},
        )
    ...
    def to_operation(self, inputs, outputs, **kwargs):
        ...
        nodes = [
            gn.PickN(
                name=self.name() + "_picked", inputs=inputs, outputs=picked_outputs, N=self.values["events"], **kwargs
            ),
            gn.Map(
                name=self.name() + "_operation",
                inputs=picked_outputs,
                outputs=outputs,
                func=McastProc(self.values["Multicast Group"], self.values["Port"]),
                **kwargs,
            ),
        ]
    ...
    class Caput(CtrlNode):
        ...
        def __init__(self, name):
            super().__init__(
                name,
                terminals={
                    "In": {"io": "in", "ttype": Union[str, int, float, Array1d]},
        ...
        def to_operation(self, inputs, outputs, **kwargs):
            ...
            nodes = [
                gn.PickN(
                    name=self.name() + "_picked",
                    inputs=inputs,
                    outputs=picked_outputs,
                    N=self.values["events"],
                    **kwargs,
                ),
                gn.Map(
                    name=self.name() + "_operation",
    ...
    class Pvput(CtrlNode):
        ...
        def __init__(self, name):
            super().__init__(
                name,
                terminals={
                    "In": {"io": "in", "ttype": Union[str, int, float, Array1d, Array2d]},
                    "eventid": {"io": "in", "ttype": int},
                },
            )

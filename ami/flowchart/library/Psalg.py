import numpy as np
from amitypes import Array1d, Array2d, Array3d, DataSource, Detector
...
try:
    ...
    class CFD(CtrlNode):
        ...
        nodeName = "CFD"
        uiTemplate = [
            ("Sample Interval", "doubleSpin", {"value": 1, "min": 0.01}),
        ...
        def __init__(self, name):
            super().__init__(
                name, terminals={"In": {"io": "in", "ttype": Array1d}, "Out": {"io": "out", "ttype": float}}
            )
...
try:
    ...
    class XTCAVLasingOn(CtrlNode):
        ...
        uiTemplate = [
            ("num bunches", "intSpin", {"value": 1, "min": 1}),
        ...
        def to_operation(self, **kwargs):
            locpars = {
                "num_bunches": self.values["num bunches"],
...
try:
    ...
    class PeakFinder1D(CtrlNode):
        ...
        uiTemplate = [("threshold lo", "doubleSpin", {"value": 0}), ("threshold hi", "doubleSpin", {"value": 1})]
        ...
        def __call__(self, calib):
            ...
            return (
                ...
                if self.mask is None
                    ...
                    (self.mask, None)
                    if self.mask.ndim == 2
                    else (None, self.mask) if self.mask.ndim == 3 else (None, None)
                )
            )
    ...
    class Mask(CtrlNode):
        ...
        def __init__(self, name):
            ...
            super().__init__(
                name,
                terminals={
                    "calibconst": {"io": "in", "ttype": dict},
                    "Mask": {"io": "out", "ttype": Array2d},
                    "Mask3D": {"io": "out", "ttype": Array3d},
                },
            )
            logger.info(
                "__init__: %s" % self.__init__.__doc__.rstrip()
        ...
        def dict_mask_pars_from_values(self):
            ...
            try:
                d[k] = (
                    np.load(fname)
                    if fname and ext == "npy"
                    else load_txt(fname) if fname and ext in ("data", "dat", "text", "txt") else None
                )
    ...
    class GeometryProd:
        ...
        def __call__(self, calib, arr3d=None):  # mask2d=None):
            ...
            if self.do_load_geo > 0:  # =0 - do not load, =1 - from file, =2 - from DB
                ...
                logger.debug(
                    "\n  %s\n  %s\n  %s\n  %s"
                    % (info_ndarr(ix, "ix:"), info_ndarr(iy, "iy:"), info_ndarr(x, " x:"), info_ndarr(y, " y:"))
                )

                img = (
                    None if arr3d is None else img_from_pixel_arrays(ix.ravel(), iy.ravel(), W=arr3d.ravel())
                )  # dtype=np.float32, vbase=0
...
try:
    ...
    class Mask3dFrom2dProd:
        ...
        def __call__(self, inds_xy, mask2d):
            ...
            logger.info(
                "Mask3dFrom2dProd\n  %s\n  %s\n  %s"
                % (info_ndarr(ix, "ix:"), info_ndarr(iy, "iy:"), info_ndarr(mask2d, "input mask2d:"))
            )
...
try:
    from ami.pyalgos.NDArrUtils import arr_rot_n90, reshape_to_2d  # info_ndarr
    from ami.pyalgos.PSUtils import table_nxm_jungfrau_from_ndarr, table_nxn_epix10ka_from_ndarr
    ...
    class TableFromArr3dProd:
        ...
        def __call__(self, arr3d):
            ...
            arr2d = (
                table_nxm_jungfrau_from_ndarr(arr3d)
                if (len(arr3d) % 512 * 1024) == 0
                else table_nxn_epix10ka_from_ndarr(arr3d) if (len(arr3d) % 384) == 0 else reshape_to_2d(np.array(arr3d))
            )
    ...
    class TableFromArr3d(CtrlNode):
        ...
        def __init__(self, name):
            """constructor - called at droppong CtrlNode on flowchart'."""
            super().__init__(
                name, terminals={"arr3d": {"io": "in", "ttype": Array3d}, "arr2d": {"io": "out", "ttype": Array2d}}
            )
...
class ThresholdingHitFinder(CtrlNode):
    ...
    def __init__(self, name):
        super().__init__(
            name,
            terminals={"In": {"io": "in", "ttype": Array2d}, "Out": {"io": "out", "ttype": Array2d}},
            global_op=True,
        )
    ...
    def to_operation(self, inputs, outputs, **kwargs):
        ...
        if fct == "Exponential Moving Average":
            ...
            nodes = [
                gn.Map(name=self.name() + "_map", inputs=inputs, outputs=mapped_outputs, func=threshold_img, **kwargs),
                gn.Accumulator(
            ...
            nodes = [
                gn.Map(name=self.name() + "_map", inputs=inputs, outputs=mapped_outputs, func=threshold_img, **kwargs),
                gn.SumN(
                    name=self.name() + "_accumulated", inputs=mapped_outputs, outputs=summed_outputs, N=N, **kwargs
                ),
                gn.Map(
                    name=self.name() + "_unzip",
                    inputs=summed_outputs,

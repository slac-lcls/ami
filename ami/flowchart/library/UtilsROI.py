...
def handle_positions_normalized(radius_out, radius_int, angle_deg_out, angle_deg_int):
...
class ArchROI(pg.ROI):
    ...
    def shape_parameters(self):
        ...
        fmt = (
            "ArchROI.pos: (%.1f, %.1f) size(%.1f, %.1f) rad1:%.1f " + "rad2:%.1f angle1:%.1f angle1:%.2f center view:%s"
        )

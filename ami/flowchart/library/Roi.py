...
from ami.flowchart.library.DisplayWidgets import ImageWidget, PixelDetWidget, ScatterWidget, WaveformWidget
...
try:
    ...
    class PolarHistogram:
        ...
        def __call__(self, img, mask=None):
            ...
            if self.hpolar is None:
                logger.info(
                    "update hpolar with cx:%.1f, cy:%.1f, ro:%d, ri:%d, ao:%.1f, ai:%.1f, nr:%d, na:%d"
    ...
    class RoiArch(CtrlNode):
        ...
        nodeName = "RoiArch"
        uiTemplate = [
            ("center x", "intSpin", {"value": 200, "min": -1000}),
        ...
        def display(self, topics, terms, addr, win, **kwargs):
            ...
            if self.widget:
                ...
                width = 4
                kwargs = {
                    "handlePen": QPen(QBrush(QColor("yellow")), width),
        ...
        def shape_values(self):
            return [self.values[s] for s in ("center x", "center y", "radius o", "radius i", "angdeg o", "angdeg i")]
        ...
        def to_operation(self, inputs, outputs, **kwargs):
            ...
            logger.info(
                "to_operation cx:%.1f, cy:%.1f, ro:%d, ri:%d, ao:%.1f, ai:%.1f, nr:%d, na:%d"
                % (cx, cy, ro, ri, ao, ai, nr, na)
            )
            return gn.Map(
                name=self.name() + "_operation", inputs=inputs, outputs=outputs, func=PolarHistogram(*args), **kwargs
            )
...
class Roi2D(CtrlNode):
    ...
    nodeName = "Roi2D"
    uiTemplate = [
        ("origin x", "intSpin", {"value": 0, "min": 0}),
    ...
    def display(self, topics, terms, addr, win, **kwargs):
        ...
        if self.widget:
            self.roi = pg.RectROI(
                [self.values["origin x"], self.values["origin y"]], [self.values["extent x"], self.values["extent y"]]
            )
            self.roi.sigRegionChangeFinished.connect(self.set_values)
            self.widget.view.addItem(self.roi)
...
class Roi1D(CtrlNode):
    ...
    nodeName = "Roi1D"
    uiTemplate = [("origin", "intSpin", {"value": 0, "min": 0}), ("extent", "intSpin", {"value": 10, "min": 1})]
    ...
    def display(self, topics, terms, addr, win, **kwargs):
        ...
        if self.widget:
            self.roi = pg.LinearRegionItem(
                (self.values["origin"], self.values["extent"]), brush=fn.mkBrush(0, 255, 0, 100), swapMode="None"
            )
...
class ScatterRoi(CtrlNode):
    ...
    def display(self, topics, terms, addr, win, **kwargs):
        ...
        if self.widget:
            self.roi = pg.LinearRegionItem(
                (self.values["origin"], self.values["extent"]), swapMode="sort", brush=fn.mkBrush(0, 255, 0, 100)
            )
            self.widget.plot_view.addItem(self.roi)
            self.roi.sigRegionChangeFinished.connect(self.set_values)
    ...
    def to_operation(self, inputs, outputs, **kwargs):
        ...
        nodes = [
            gn.PickN(
                name=self.name() + "_pickN", inputs=inputs, outputs=pickn_outputs, **kwargs, N=self.values["Num Points"]
            ),
            gn.Map(name=self.name() + "_operation", inputs=pickn_outputs, outputs=outputs, func=func, **kwargs),
            gn.Map(
...
class Roi0D(CtrlNode):
    ...
    nodeName = "Roi0D"
    uiTemplate = [("x", "intSpin", {"value": 0, "min": 0}), ("y", "intSpin", {"value": 0, "min": 0})]

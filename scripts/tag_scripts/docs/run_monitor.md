...
```
ok      xpp  /…/cron_logs/xpp/ami_branch/2026-09-26_020000.log
ok      tmo  /…/cron_logs/tmo/ami_branch/2026-09-26_020000.log
=== finished …: 0 failed ===
```

One line per hutch goes to stdout, and cron appends it to `run_monitor_ami.log`. If anything failed, one email goes to `MAIL_TO` listing each failure and its log, and the script exits `1`. No email is sent in a dry run.
